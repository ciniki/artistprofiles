...
function ciniki_artistprofiles_artistUpdate(&$ciniki) {
    ...
    $rc = ciniki_artistprofiles_checkAccess($ciniki, $args['tnid'], 'ciniki.artistprofiles.artistUpdate');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    if( isset($args['name']) ) {
        ...
        if( $rc['num_rows'] > 0 ) {
            return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.artistprofiles.21', 'msg'=>'You already have an artist with this name, please choose another name'));
        }
    }
    ...
    $rc = ciniki_core_objectUpdate($ciniki, $args['tnid'], 'ciniki.artistprofiles.artist', $args['artist_id'], $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
